/**
 * @file Type definitions required for the server application
 * @author François Nguyen <https://github.com/lith-light-g>
 */

/// <reference path="../ropeho.d.ts" />
/// <reference types="express" />
/// <reference types="express-serve-static-core" />
/// <reference types="serve-static" />
/// <reference types="mime" />
/// <reference types="passport" />
/// <reference types="passport-local" />
/// <reference types="passport-facebook" />
/// <reference types="express-session" />
/// <reference types="body-parser" />
/// <reference types="node" />
/// <reference types="lodash" />
/// <reference types="validator" />
/// <reference types="uuid" />
/// <reference types="cors" />
/// <reference types="moment" />
/// <reference types="nodemailer" />
/// <reference types="ejs" />
/// <reference types="aws-sdk" />
/// <reference types="rimraf" />
/// <reference types="mkdirp" />
/// <reference types="fluent-ffmpeg" />
/// <reference types="redis" />
/// <reference types="kue" />
/// <reference types="socket.io" />
/// <reference types="sharp" />
/// <reference types="connect-redis" />
/// <reference types="pluralize" />
/// <reference types="cookie-signature" />
/// <reference path="../../definitions/tmp/index.d.ts" />
